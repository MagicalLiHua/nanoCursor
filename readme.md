...
nanoCursor 是一个探索性的多智能体（Multi-Agent）自动编程协作框架。基于 LangGraph 状态机构建有向图工作流，将大模型剥离为职责独立的 Planner（架构规划）、Coder（代码修改）与 Reviewer（报错诊断）三大智能体，实现了从"需求理解 -> 架构规划 -> 局部编码 -> 沙盒测试 -> 代码审查与自动修复"的全自动闭环流转。
...
基于 LangGraph 构建 DAG 工作流，每个节点职责单一、边界清晰：
...
- **AST 感知读取**：Python 标准库 `ast` 解析，大文件返回结构大纲，支持按函数/类/行号精确提取
- 文件签名索引机制：提取函数/类签名，减少不必要的文件读取
- 文件备份与回滚：每次修改自动备份，支持一键回滚
...
| `route_after_coder` | 有 tool_calls | `coder_step_counter` -> `coder_tools` -> `coder` |
...
│   │   ├── context_manager.py  # 上下文管理器 v2.0
│   │   ├── state.py        # Agent 状态定义 + MemorySaver
│   │   ├── llm_engine.py   # LLM 引擎初始化
│   │   ├── logger.py       # 日志系统配置
│   │   └── repo_map.py     # 仓库目录树 + 文件签名生成
│   │
│   └── tools/
│       └── file_tools.py   # 文件操作工具 (read/edit/write/list/rollback)
│
...
### 5. AST 感知的智能文件读取 (AST-Aware File Reading)

传统方案直接读取整个文件内容，当文件过大时会被强制压缩，导致 Agent 丢失中间代码上下文，`Search/Replace` 匹配频繁失败。

nanoCursor 的分层读取策略：

```
┌─────────────────────────────────────┐
│  read_file(filename)                 │  <- 小文件返回完整内容，大文件返回 AST 结构大纲
├─────────────────────────────────────┤
│  read_function(filename, name)       │  <- AST 精确定位，提取指定函数完整源码
├─────────────────────────────────────┤
│  read_class(filename, name)          │  <- AST 精确定位，提取指定类完整源码
├─────────────────────────────────────┤
│  read_file_range(filename, s, e)     │  <- 按行号范围读取，带行号前缀
└─────────────────────────────────────┘
```

技术实现：

- 使用 Python 标准库 `ast` 解析代码抽象语法树，无需额外依赖
- 大文件自动识别（> 5000 字符），返回结构大纲而非原始内容
- 大纲包含所有函数/类的名称、参数、起始行号与结束行号
- Agent 可根据大纲按需调用 `read_function` 精确提取目标代码
...
- [ ] **基于 AST 的代码修改 (libcst/Tree-sitter)**：当前 `edit_file` 仍基于纯文本查找替换，在面对 LLM 缩进/空行幻觉时仍显脆弱。计划引入 libcst 将代码解析为具体语法树，实现函数/类级别的结构化精准修改，从根源消除 `Search/Replace` 匹配失败。
