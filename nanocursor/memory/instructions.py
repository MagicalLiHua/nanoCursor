...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.nanocursor/NANOCURSOR.md, ~/.nanocursor/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 NANOCURSOR.md 和 AGENTS.md
    3. workDir/.nanocursor/INSTRUCTIONS.md（遗留格式）
    ...
    for d in _project_instruction_dirs(root):
        _add(d / "NANOCURSOR.md")
        _add(d / "AGENTS.md")

    # 3. 遗留格式
    _add(root / ".nanocursor" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "NANOCURSOR.local.md")
