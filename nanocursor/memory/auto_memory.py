...
def get_auto_mem_path(project_root: str) -> str:
    """返回项目级记忆目录路径：<projectRoot>/.nanocursor/memory/。

    保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
    支持 NANOCURSOR_REMOTE_MEMORY_DIR 环境变量覆盖。
    """
    override = os.environ.get("NANOCURSOR_REMOTE_MEMORY_DIR", "")
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".nanocursor", "memory") + os.sep


def get_user_auto_mem_path() -> str:
    """返回用户级记忆目录路径：~/.nanocursor/memory/。
...
class MemoryManager:
    ...
    def __init__(self, project_root: str) -> None:
        ...
        # 用户级：~/.nanocursor/memory/ — user/feedback 类型记忆
        self._user_mem_dir = get_user_auto_mem_path()
        # 项目级：<projectRoot>/.nanocursor/memory/ — project/reference 类型记忆
    ...
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.nanocursor/memory/）。"""
