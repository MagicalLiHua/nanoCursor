...
需要环境变量 NANOCURSOR_TEST_API_KEY、NANOCURSOR_TEST_BASE_URL、NANOCURSOR_TEST_MODEL。
...
def test_lock_first_acquire():
    from nanocursor.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from nanocursor.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from nanocursor.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from nanocursor.memory.consolidation import _build_consolidation_prompt
...
@pytest.mark.skipif(
    not os.environ.get("NANOCURSOR_TEST_API_KEY"),
    reason="NANOCURSOR_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["NANOCURSOR_TEST_API_KEY"]
    base_url = os.environ.get("NANOCURSOR_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("NANOCURSOR_TEST_MODEL", "MiniMax-M3")

    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".nanocursor", "memory")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from nanocursor.tools.read_file import ReadFile
    from nanocursor.tools.write_file import WriteFile

    from nanocursor.config import ProviderConfig
    from nanocursor.client import OpenAICompatClient
    ...
    from nanocursor.permissions.rules import RuleEngine
    from nanocursor.permissions.dangerous import DangerousCommandDetector
    from nanocursor.permissions.checker import PermissionMode
