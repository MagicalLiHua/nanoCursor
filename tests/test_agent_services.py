...
def test_create_conversation_starts_with_lead_only_team(tmp_path):
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    assert loaded["team"]["source"] == "lead_only"
    assert [member["name"] for member in loaded["team"]["members"]] == ["Lead"]
    assert loaded["team"]["members"][0]["last_action"] == "等待用户输入；必要时会为任务创建子 Agent。"
...
def test_compose_runtime_team_scales_with_complexity(tmp_path):
    from src.api.services.conversation_service import compose_runtime_team

    workspace = tmp_path / "workspace"
    workspace.mkdir()

    simple = compose_runtime_team("你好，解释一下这个项目", str(workspace), "conv-1")
    small = compose_runtime_team("帮我修复 README 里的一个拼写错误", str(workspace), "conv-1")
    python_script = compose_runtime_team("帮我用python写常见的排序算法并比较性能", str(workspace), "conv-1")
    review_only = compose_runtime_team("帮我复核 Diff 风险", str(workspace), "conv-1")
    medium = compose_runtime_team("实现一个完整的前端任务面板并更新接口文档", str(workspace), "conv-1")
    high = compose_runtime_team("重构认证权限并处理数据库 schema 迁移和回滚风险", str(workspace), "conv-1")

    assert [member["name"] for member in simple["members"]] == ["Lead"]
    assert [member["name"] for member in small["members"]] == ["Lead", "Coder"]
    assert [member["name"] for member in python_script["members"]] == ["Lead", "Coder"]
    assert "Reviewer" in [member["name"] for member in review_only["members"]]
    assert [member["name"] for member in medium["members"]] == ["Lead", "Planner", "Coder", "Reviewer"]
...
def test_build_delivery_report_skips_lightweight_conversation(tmp_path):
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    store = EventStore()
    store.create_session("hello-run", "你好", str(workspace), status="completed")
    store.update_session(
        "hello-run",
        str(workspace),
        execution_plan={
            "strategy": "lead_direct_reply",
            "stages": [{"id": "lead_reply", "status": "completed"}],
        },
    )
    store.append_event(
        "hello-run",
    ...
    report = build_delivery_report("hello-run", str(workspace))
    center = build_artifact_center("hello-run", str(workspace))
    artifacts = {item["id"]: item for item in center["artifacts"]}

    assert report["source"] == "not_applicable"
    assert report["markdown"] == ""
    assert artifacts["report"]["status"] == "empty"
...
def test_artifact_center_collects_demo_delivery_artifacts(tmp_path):
    ...
    assert center["summary"]["artifact_count"] == 11
    ...
    assert artifacts["parallel_proposals"]["status"] == "empty"
    assert artifacts["parallel_merge_plan"]["status"] == "empty"
...
def test_context_pack_includes_file_outlines_and_plan(tmp_path):
    ...
    pack = build_context_pack(
        prompt="修复 login 测试",
        workspace_dir=str(workspace),
        execution_plan={"stages": [{"id": "verify", "title": "验证", "description": "运行测试"}]},
    )

    assert "app.py" in pack.relevant_files
    assert any(item["path"] == "app.py" for item in pack.file_outlines)
    assert "handle_login" in pack.symbols
    assert pack.current_plan[0]["title"] == "验证"
    assert "文件大纲" in pack.to_text()
