...
from nanocursor.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from nanocursor.memory.session import (
    RecordType,
    ResumeResult,
    Session,
    SessionManager,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        nanocursor_md = tmp_path / "NANOCURSOR.md"
        nanocursor_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """对齐 Go 版发现顺序：NANOCURSOR.md 在前，.nanocursor/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "NANOCURSOR.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".nanocursor"
        dotdir.mkdir()
        # Go 版不发现 .nanocursor/NANOCURSOR.md，只发现 .nanocursor/INSTRUCTIONS.md（legacy）
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".nanocursor" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".nanocursor" / "memory"
...
class TestConversationInjection:
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "nanocursorMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型与 Go 版一致。"""
        from nanocursor.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES
