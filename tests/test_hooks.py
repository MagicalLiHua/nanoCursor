...
class TestPromptExecutor:
    ...
    async def test_returns_message(self):
        from nanocursor.hooks.executors import execute_prompt
...
class TestHttpExecutor:
    ...
    async def test_mock_request(self):
        from nanocursor.hooks.executors import execute_http

        action = Action(type="http", url="https://httpbin.org/post", body='{"test": true}')
        ctx = HookContext()
        # 用 mock 避免发起真实的网络请求
        with patch("nanocursor.hooks.executors.urlopen") as mock_urlopen:
...
class TestAgentExecutor:
    ...
    async def test_stub(self):
        from nanocursor.hooks.executors import execute_agent
...
class TestAgentHookIntegration:
    ...
    async def test_pre_tool_use_reject_skips_tool(self):
        from nanocursor.agent import Agent, ToolResultEvent
        ...
        from nanocursor.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete
