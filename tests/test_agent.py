...
from nanocursor.serialization import build_anthropic_messages
...
async def test_single_step_tool_call():
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "ReadFile", {"file_path": "NANOCURSOR.md"}),
    ...
    conv.add_user_message("Read NANOCURSOR.md")
...
async def test_multi_step_autonomous():
    ...
    test_file = "/tmp/nanocursor_test_hello.txt"
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "WriteFile", {"file_path": "/tmp/nanocursor_test_hello.txt", "content": "Hello World"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # 第 2 轮：ReadFile 进行验证
        [
            TextDelta("Verifying content."),
            ToolCallComplete("t2", "ReadFile", {"file_path": "/tmp/nanocursor_test_hello.txt"}),
...
async def test_stop_max_iterations():
    ...
    for i in range(5):
        responses.append([
            TextDelta(f"Step {i}"),
            ToolCallComplete(f"t{i}", "ReadFile", {"file_path": "NANOCURSOR.md"}),
...
async def test_stop_cancel():
    ...
    class SlowMockClient(LLMClient):
        ...
        async def stream(
            ...
            await asyncio.sleep(0.01)
            yield ToolCallComplete(f"t{self._call_count}", "ReadFile", {"file_path": "NANOCURSOR.md"})
            await asyncio.sleep(0.01)
...
async def test_token_usage_accumulates():
    ...
    client = MockLLMClient([
        ...
        [
            TextDelta("Step 2"),
            ToolCallComplete("t2", "ReadFile", {"file_path": "NANOCURSOR.md"}),
