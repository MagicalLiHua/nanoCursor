...
from nanocursor.client import resolve_context_window
from nanocursor.config import ProviderConfig
from nanocursor.validator import (
    ConfigError,
...
class TestAutoFetch:
    ...
    async def test_fetch_success_is_cached_and_used(self):
        ...
        with patch("nanocursor.client.create_client", return_value=fake) as mk:
    ...
    async def test_client_construction_failure_degrades(self):
        ...
        p = _provider(model="claude-sonnet-4-6")
        with patch(
            "nanocursor.client.create_client",
    ...
    async def test_non_anthropic_provider_is_not_fetched(self):
        ...
        with patch("nanocursor.client.create_client") as mk:
            await resolve_context_window(p)
