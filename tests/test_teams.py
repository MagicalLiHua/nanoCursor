...
from nanocursor.teams.backend_detect import BackendDetectionError, detect_backend, detect_pane_backend
from nanocursor.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from nanocursor.agents.tool_filter import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("nanocursor.teams.models.Path.home", return_value=Path(tmp_dir)):
            ...
            (Path(tmp_dir) / ".nanocursor" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_pane_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("nanocursor.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_pane_no_backend_falls_back_to_in_process(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("nanocursor.teams.backend_detect.shutil.which", return_value=None):
                result = detect_pane_backend()
                assert result == BackendType.IN_PROCESS
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from nanocursor.config import AppConfig
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from nanocursor.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from nanocursor.teams.transcript import load_transcript
        with patch("nanocursor.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from nanocursor.prompts import build_system_prompt, IDENTITY_SECTION
        prompt = build_system_prompt()
        # 验证 identity section 内容包含在 prompt 中
        assert "NanoCursor" in prompt
