...
def run_eval_with_command(
    ...
    *mode*:
    ...
    # Agent simulation is intentionally limited to agent mode. ``baseline`` and
    # ``command_only`` keep the original fixture state so ablation can observe
    # what happens without runtime intervention.
    if mode == "agent":
...
def _emit_agent_events(
    ...
    if task.get("fixture") and required_files:
        ...
        _write_fixture_solution(task, target)
...
def _write_fixture_solution(task: dict[str, Any], target: Path) -> None:
