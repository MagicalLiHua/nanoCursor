"""Read nanoCursor task and team state from workspace files."""
...
DEFAULT_TEAM = [
    {
        ...
        "role": "lead",
        "status": "idle",
        "goal": "Coordinate the run, decide which agents are needed, and keep the outcome aligned with the user request.",
        "tools": ["plan", "delegate", "spawn_agent", "report"],
        "capabilities": ["tool.memory", "tool.project_index"],
        "current_task_id": None,
        "last_action": "等待用户输入；必要时会创建临时或永久 Agent。",
...
def add_team_member(
    ...
    member = {
        ...
        "current_task_id": None,
        "last_action": "Lead 创建了永久 Agent 角色卡。" if source == "lead" else "用户创建了自定义 Agent 角色卡。",
        "artifacts": [],
        "last_active_at": now,
        "updated_at": now,
        "source": source,
