...
# 注意：沙箱会自动发现所有 test_*.py 文件和其中的 Test* 类/test_* 函数。
# 不需要手动入口，直接运行 pytest 或 unittest discover 即可。
# 如果需要独立手动运行，可以使用: python -m unittest test_quicksort.py -v

if __name__ == "__main__":
    # 运行单元测试
    print("运行快速排序单元测试...")
    # 使用 unittest discover 模式运行所有测试
