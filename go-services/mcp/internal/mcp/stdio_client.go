...
func newStdioClient(ctx context.Context, cfg ProbeRequest) (*stdioClient, error) {
	process, err := StartStdioProcess(ctx, cfg.Command, cfg.Args, cfg.WorkspaceDir, cfg.Env)
	if err != nil {
		return nil, err
	}
	client := &stdioClient{
	...
	if _, err := client.request(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "nanocursor-mcp", "version": "0.1.0"},
	}); err != nil {
		_ = client.Close()
...
// classifyToolPermission returns a permission level string for the given MCP tool name.
func classifyToolPermission(name string) string {
	lower := strings.ToLower(name)
	switch {
	case strings.Contains(lower, "read") || strings.Contains(lower, "list") || strings.Contains(lower, "get") || strings.Contains(lower, "search") || strings.Contains(lower, "fetch"):
		return "mcp_read"
	case strings.Contains(lower, "write") || strings.Contains(lower, "create") || strings.Contains(lower, "update") || strings.Contains(lower, "edit") || strings.Contains(lower, "delete"):
		return "mcp_write"
	case strings.Contains(lower, "run") || strings.Contains(lower, "execute") || strings.Contains(lower, "call") || strings.Contains(lower, "send"):
		return "mcp_execute"
	default:
		return "mcp_write"
	}
